...
Pester 5.7.1 では成功していた MCP 検査の4件が、CI の Pester 6.2.0 では
部分 Mock に一致しない `Get-Command` 呼び出しで失敗した。
依存欠落と初期応答なしの条件を、実コマンド用の PATH フィクスチャで再現する方式に変更した。
PATH は `finally` で復元し、タイムアウト用の shell は `exec` で子を残さず sleep に置換する。
修正後の近傍17件は Pester 5.7.1 / 6.2.0 の両方で成功。
Pester 6.2.0 の全件検証は438成功、0失敗、実 Codex E2E 1件スキップ。
最終 CI は GitHub の対象 SHA を正本とする。

### 運用上の制限

PR レビューで、MCP の `ReadToEndAsync()` が出力サイズに比例してメモリを保持する問題を確認した。
4096文字の非同期チャンク読み取りへ変更し、stdout / stderr の合計保持を1,048,576文字に制限した。
超過時は部分出力を返さず失敗とし、既存の専用プロセスグループ回収を実行する。
初期応答も32文字未満に制限した。上限ちょうど・各ストリームと合計の超過・子の回収・
unhealthy 判定・不正初期応答を追加し、近傍24件は Pester 5.7.1 で成功した。
README と移植文書に回収対象外の条件も同期した。
