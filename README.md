...
- MCP の `healthCommand` は Linux の `TEMP` 未設定環境に対応し、引数を個別に渡します。標準出力と標準エラーを並行して読み、一時ファイルには保存しません。`setsid`、`sh`、外部 `kill` と `/proc` を使用して専用プロセスグループを管理し、タイムアウト時は残った子も終了します。必要なコマンドがない場合は検査を実行せず失敗を返します。
