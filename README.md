...
| 番号 | アイコン | 機能 | 説明 |
|---:|---|---|---|
| `L1`, `L2`, ... | 🚀 | Codexを起動 | 設定されたルートごとの候補からCodexを起動 |
...
週間利用枠の残り50％以下で指定 Goal の自動継続を停止するには、Bash から
`python3 -B scripts/main/codex_weekly_guard.py --thread THREAD_ID --remaining 50` を実行します。
PowerShell は不要です。読み取りだけなら `--check` を指定します。
実行中ターン・別セッションの消費は停止対象外で、厳密な50％確保を保証するものではありません。
設定・失敗時の動作・検証方法は [週間利用枠による Goal 停止](docs/migration/weekly-quota-guard.md) を参照してください。

- `Start-CodexBootstrap.ps1 -DryRun` と `Start-Codex.ps1 -DryRun` は、初回の設定未生成時にも隣接する `config.json.template` を検証します。設定・状態・履歴・ログを作成せず、ログの削除も行いません。
- プロジェクトの番号選択はフルパスを保持し、同名プロジェクトでも選んだ対象だけを起動・Supervisor 適用します。CLI の `-Project` は絶対パスにも対応します。曖昧な旧形式の名前指定は拒否し、新しい履歴にはフルパスを保存します。
- MCP の `healthCommand` は Linux の `TEMP` 未設定環境に対応し、引数を個別に渡します。標準出力と標準エラーは合計1,048,576文字（UTF-16コード単位）まで並行して読み、一時ファイルには保存しません。超過時は部分出力を返さず、固定のエラー文と失敗終了コードを返します。`setsid`、`sh`、外部 `kill` と `/proc` を使用して専用プロセスグループを管理し、タイムアウト・出力超過時はグループ内に残った子も終了します。別セッションへ離脱した子や、出力を閉じて正常終了したコマンドが残すバックグラウンドプロセスは回収対象外です。必要なコマンドがない場合は検査を実行せず失敗を返します。
- リリース前チェックは Pester の検出失敗、テストなし、成功テストなしを失敗と判定します。
- CI は state の JSON Schema と設定テンプレートの既存 validator を実行し、Pester の失敗・lint Error・モジュール依存欠落を拒否します。既存 lint Warning は表示を継続します。

詳細な変更理由、検証範囲、外部連携の適用有無は [実利用可能性の検証記録](docs/analysis/runtime-readiness-2026-09-12.md) を参照してください。
